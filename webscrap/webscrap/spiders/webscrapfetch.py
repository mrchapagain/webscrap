...
class ExtractUrls(scrapy.Spider):
    ...
    def parse_product_category_pages(self, response):
        # link for going to next pages
        nextpage_product_subcategory_links = response.css('div.products > div.category-slider > a::attr(href)').getall() # Output will be: ['/dagligvarer/frugt','/dagligvarer/frugt/frugtkasser','/dagligvarer/frugt/aebler-paerer','/dagligvarer/frugt/baer-druer','/dagligvarer/frugt/citrusfrugter','/dagligvarer/frugt/eksotiske-frugter', '/dagligvarer/frugt/blandede-frugtkasser']
    ...
    def parse_product_details_pages(self, response): # response from: <GET https://www.aarstiderne.com/dagligvarer/frugt/frugtkasser/dragefrugter-i-kasse>
        # link for going to next pages
        nextpage_Aarstidernes_anbefalers_links = ['/dagligvarer/anbefalinger'] ##response.css('li#menu-item-bundles > a::attr(href)').getall() # Output will be: '/jordens-bedste-koebmand/anbefalinger'
    ...
    def parse_aarstidernes_anbefalers_detail_pages(self, response):   # response from <GET https://www.aarstiderne.com/dagligvarer/anbefalinger/hokkaidosuppe>
        # link for going to next pages
        nextpage_metadata_links = response.css('div.header__top > nav > ul > li:nth-child(6) > a::attr(href)').getall() # Output will be: ['/om-aarstiderne']
    ...
    def parse_product_company_metadata_pages(self, response): # response from <GET https://www.aarstiderne.com/om-aarstiderne> 
        # link for going to next pages
        nextpage_finalpage_links = ['/om-aarstiderne/baeredygtighed-og-miljoe', '/om-aarstiderne/innovation-og-produktudvikling'] # use how to select multiple nth in css selector
        # response.css('div.products-bg > section > article > nav > div:nth-child(4-5) > a::attr(href)').getall() # Output will be: ['/om-aarstiderne/baeredygtighed-og-miljoe', '/om-aarstiderne/innovation-og-produktudvikling']
    ...
    def parse_data_save_pages(self, response): # Response from: <GET https://www.aarstiderne.com/om-aarstiderne/baggrund-og-tal/aarstidernes-5-loefter>
        ...
        with open(json_file, "wb") as fout:
            line=json.dumps(dict(items), ensure_ascii=True,  indent= '\t') + "\n"  
