...
def save_to_log_file(file_name, parce_func, start_urls, start_url, parce_urls, parce_to):
    with open (file_name, "a") as flog:
        ...
        flog.writelines({"\t" + i + "\n" for i in start_urls}) #map(lambda x: 'https://www.aarstiderne.com' + x + "\n", start_url)
        flog.writelines(f"parce_urls: {parce_urls} to: {parce_to}"+ "\n")
        flog.writelines("------------------------------------------------------------ END" + "\n")
...
class ExtractUrls(scrapy.Spider):
    ...
    def parse_frontpage_to_tabnavigation_page(self, response):
        ...
        #with open (json_file, "wb") as fout:
            #line=json.dumps(dict(request.meta['items']), ensure_ascii=True,  indent= '\t') + "\n"  
    ...
    def parse_product_company_metadata_pages(self, response):
        ...
        # Save information to log file and print print
        save_to_log_file('log.jsonlines', "parse_product_company_metadata_pages", [nextpage_finalpage_links], response, "None", "None")
